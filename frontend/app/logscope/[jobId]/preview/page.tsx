...
export default function LogScopePreviewPage({ params }: { params: Promise<{ jobId: string }> }) {
  ...
  const downloadDocxUrl = `/api/logscope/export/${jobId}.docx`;

  const fetchPdf = async () => {
    setLoading(true);
    setError('');
    try {
      // Fetch as JSON with Base64 payload to prevent any browser extension (like IDM) from intercepting the HTTP request
      const res = await fetch(`/api/logscope/preview/${jobId}?format=json`, {
        headers: {
      ...
      // Convert Base64 string directly into an in-memory Uint8Array
        ...
        <div className="flex flex-wrap justify-between items-center gap-3 bg-white/[0.02] border border-white/5 rounded-2xl px-5 py-3">
          <div className="flex items-center gap-3">
            ...
            <p className="text-xs text-slate-400 flex items-center gap-1.5">
              ...
              <span>{t('logscope.preview_exact')}</span>
            </p>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <a
              className="px-4 py-2 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-dark-950 text-xs font-bold flex items-center gap-1.5 transition-colors shadow-lg shadow-emerald-500/20"
              href={downloadDocxUrl}
            >
              <Download className="w-3.5 h-3.5" />
              <span>{t('logscope.download_previewed_word')}</span>
            </a>
          </div>
        </div>
        ...
        <div className="flex-1 relative w-full rounded-2xl overflow-hidden border border-white/10 bg-dark-900/60 shadow-2xl flex flex-col">
          ...
          {error && !loading && (
            <div className="absolute inset-0 z-10 flex flex-col items-center justify-center p-8 text-center bg-dark-950/90">
              ...
              <button
                onClick={fetchPdf}
                className="px-4 py-2 rounded-xl bg-emerald-500 text-dark-950 text-xs font-bold flex items-center gap-2 hover:bg-emerald-400 transition-colors"
              >
