...
export default function ThreatScopePreviewPage({ params }: { params: Promise<{ jobId: string }> }) {
  const { jobId } = use(params);
  const [blobUrl, setBlobUrl] = useState<string | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState('');

  const downloadDocxUrl = `/api/threatscope/export/${jobId}.docx`;

  const fetchPdf = async () => {
    setLoading(true);
    setError('');
    try {
      const res = await fetch(`/api/threatscope/preview/${jobId}?format=json`, {
        headers: {
      ...
      <div className="flex-1 w-full p-4 h-[calc(100vh-64px)] flex flex-col gap-3 max-w-7xl mx-auto">
        <div className="flex flex-wrap justify-between items-center gap-3 bg-white/[0.02] border border-white/5 rounded-2xl px-5 py-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <a
              className="px-4 py-2 rounded-xl bg-rose-500 hover:bg-rose-400 text-white text-xs font-bold flex items-center gap-1.5 transition-colors shadow-lg shadow-rose-500/20"
        ...
        <div className="flex-1 relative w-full rounded-2xl overflow-hidden border border-white/10 bg-dark-900/60 shadow-2xl flex flex-col">
          {loading && (
            <div className="absolute inset-0 z-10 flex flex-col items-center justify-center p-8 text-center bg-dark-950/80 backdrop-blur-sm">
              <div className="w-12 h-12 border-3 border-rose-500 border-t-transparent rounded-full animate-spin mb-4" />
              <h3 className="text-white font-bold text-base mb-1.5">جارِ إعداد وتنسيق تقرير Word الأمني...</h3>
              <p className="text-slate-400 text-xs max-w-md leading-relaxed">
                يتم استخراج كشوفات التهديدات واستخبارات VirusTotal وتنسيق صفحات المعاينة مباشرة.
              </p>
            </div>
          )}
          ...
          {error && !loading && (
            <div className="absolute inset-0 z-10 flex flex-col items-center justify-center p-8 text-center bg-dark-950/90">
              ...
              <button
                onClick={fetchPdf}
                className="px-4 py-2 rounded-xl bg-rose-500 text-white text-xs font-bold flex items-center gap-2 hover:bg-rose-400 transition-colors"
              >
          ...
          {blobUrl && (
            <iframe
              ...
              title="ThreatScope Word Report Preview"
            />
          )}
        </div>
      </div>
