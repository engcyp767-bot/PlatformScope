...
# FLOWSCOPE_CACHE_TTL_DAYS value can be used when periodic refresh is desired.
CACHE_TTL_SECONDS = max(int(os.environ.get("FLOWSCOPE_CACHE_TTL_DAYS", "0")), 0) * 24 * 60 * 60
...
def provider_configuration() -> dict[str, dict[str, Any]]:
    ...
    for item in custom_integrations.providers_for("flowscope"):
...
def enrich_ips(
    ...
    worker_count = min(max(int(os.environ.get("FLOWSCOPE_ENRICH_WORKERS", "8")), 1), len(pending_ips) or 1)
    with ThreadPoolExecutor(max_workers=worker_count, thread_name_prefix="flowscope-intel") as executor:
