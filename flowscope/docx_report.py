"""Professional, dependency-free Arabic DOCX report writer for FlowScope."""
...
def _event_reputation_result(
    ...
    related = []
    seen = set()
    for value in record.get("event_targets", []):
        ip = str(value).strip()
...
def _executive_summary(analysis: dict[str, Any]) -> str:
    ...
    if metadata.get("data_type") == "ads_events":
        types = Counter(record.get("event_type") or "غير مصنف" for record in records)
        dominant = types.most_common(1)[0] if types else ("غير مصنف", 0)
        high_risk = int(summary.get("critical", 0)) + int(summary.get("high", 0))
        _, intel_counts, checked = _intel_rows(analysis)
        if intel_counts.get("مرتفع الخطورة", 0):
            assessment = f"وأظهرت مراجعة السمعة وجود {intel_counts['مرتفع الخطورة']} مصدر ذي مؤشرات مرتفعة الخطورة تستوجب التحقق العاجل."
            ...
            assessment = "ولم تظهر على المصادر التي اكتمل فحصها مؤشرات مباشرة تثبت نشاطاً ضاراً، مع بقاء الحاجة إلى التحقق من سياق الاتصال."
        else:
            assessment = "ولم تتوفر نتائج كافية لتكوين حكم على سمعة المصادر، لذلك يبقى التقييم أولياً."
        return (
            f"أظهرت مراجعة البيانات وجود {summary.get('records', 0)} حدثاً أمنياً صادراً عن "
            f"{summary.get('unique_sources', 0)} مصدر ومتجهاً إلى {summary.get('unique_targets', 0)} وجهة. "
            f"كان {dominant[0]} النمط الأكثر تكراراً بعدد {dominant[1]} حدثاً، في حين بلغ عدد الأحداث "
            f"المصنفة مرتفعة أو حرجة الخطورة {high_risk}. {assessment} "
            "ولا تكفي درجة الخطورة أو نتيجة السمعة منفردة لإثبات وقوع اختراق دون مطابقتها مع سجلات الشبكة والأصول."
        )
    return (
        f"تم تحليل {summary.get('records', 0)} تدفقاً شبكياً بإجمالي "
        f"{summary.get('formatted_bytes', _format_bytes(summary.get('total_bytes', 0)))} و"
        f"{summary.get('total_packets', 0):,} حزمة، وشملت البيانات {summary.get('unique_ips', 0)} عنوان IP فريداً. "
        f"وشملت مراجعة سمعة المصادر {enrich.get('checked', 0)} عنواناً عاماً."
    )


def _cover(metadata: dict[str, Any]) -> str:
    settings = _report_settings()
    report_type = "أحداث أمن الشبكة (ADS)" if metadata.get("data_type") == "ads_events" else "تدفقات الشبكة"
    return "".join([
        _paragraph("بسم الله الرحمن الرحيم", align="center", bold=True, color=BLUE, size=24, before=350, after=600),
        _paragraph("تقرير تحليل أمن الشبكة", align="center", bold=True, color=BLUE, size=48, before=300, after=160),
        _paragraph("FlowScope Security Analysis Report", align="center", bold=True, color=MUTED, size=25, after=500, border_bottom=BLUE),
        ...
        _paragraph(f"إعداد: {settings.get('analyst_title') or 'فريق تحليل أمن الشبكة'} - FlowScope", align="center", bold=True, color=MUTED, size=19),
        _page_break(),
    ])


def _summary_section(analysis: dict[str, Any]) -> str:
    metadata, summary, enrich = analysis.get("metadata", {}), analysis.get("summary", {}), analysis.get("enrichment", {})
    if metadata.get("data_type") == "ads_events":
        cards = [_metric_card(summary.get("records", 0), "إجمالي الأحداث"),
                 _metric_card(summary.get("unique_sources", 0), "المصادر الفريدة"),
                 _metric_card(summary.get("critical", 0), "أحداث حرجة", RED),
                 _metric_card(summary.get("high", 0), "أحداث مرتفعة", ORANGE)]
    else:
        cards = [_metric_card(summary.get("records", 0), "إجمالي التدفقات"),
                 _metric_card(summary.get("unique_ips", 0), "عناوين IP"),
                 _metric_card(summary.get("formatted_bytes", "0 B"), "حجم البيانات", ORANGE),
                 _metric_card(f"{summary.get('total_packets', 0):,}", "إجمالي الحزم")]
    return "".join([
        ...
        _paragraph("شمل التحليل توحيد السجلات واستخراج عناوين المصدر والوجهة وحساب مؤشرات الخطورة، ثم إثراء المصادر العامة عبر الخدمات المفعلة. جرى استبعاد العناوين المحلية والخاصة من الاستعلامات الخارجية."),
        _rich_paragraph([{"text": "تغطية تحليل سمعة المصادر: ", "bold": True, "color": BLUE, "size": 22},
                         {"text": f"{enrich.get('checked', 0)} / {enrich.get('total', 0)}", "bold": True, "size": 22},
...
def _event_details_section(analysis: dict[str, Any], reader: Any) -> str:
    if analysis.get("metadata", {}).get("data_type") != "ads_events":
        return ""
    intel_results = analysis.get("enrichment", {}).get("results", {})
    records = reader.iter_records(sort_by="risk_score_desc", limit=20)
    parts = [_section_title("تفاصيل الأحداث الأعلى خطورة", page_break_before=True),
             _paragraph("يعرض كل قسم حدثاً مستقلاً مع البيانات المتاحة وخلاصة مراجعة سمعة مصدر الحدث.", color=MUTED, size=19)]
    for index, record in enumerate(records, 1):
        ...
        reputation_details = [
            _rich_paragraph([
                {"text": "الحكم الموحد: ", "bold": True, "color": BLUE, "size": 17},
                {"text": overall, "bold": True, "color": overall_color, "size": 17},
            ], after=25),
        ]
        if result_ip and result_ip != str(source):
            reputation_details.append(_paragraph(
                f"المصدر عنوان داخلي؛ توفرت نتائج لـ {related_count} عنوان عام مرتبط بالحدث، "
                f"وتعرض البطاقة النتيجة الأعلى أهمية للعنوان {result_ip}.",
                color=MUTED, size=16, after=25,
            ))
        reputation_details.extend(_source_reputation_details(intel))
        detail_rows = [[_cell(f"الحدث رقم {index}", width=1600, fill=BLUE, bold=True, color=WHITE, align="right"),
                        _cell(f"{record.get('event_type', '-')} | #{record.get('event_id', '-')}", width=7500, fill=BLUE, bold=True, color=WHITE, align="center", colspan=3)],
                       [_cell("وقت الكشف", width=1600, fill=LIGHT_BLUE, bold=True), _cell(record.get("event_time", "-"), width=2950),
                        _cell("الخطورة", width=1600, fill=LIGHT_BLUE, bold=True), _cell(f"{record.get('severity', '-')} - الدرجة {record.get('risk_score', 0)} من 100", width=2950, bold=True)],
                       [_cell("عنوان المصدر", width=1600, fill=LIGHT_BLUE, bold=True), _cell(source, width=2950),
                        _cell("حجم البيانات", width=1600, fill=LIGHT_BLUE, bold=True), _cell(_format_bytes(_record_volume(record)), width=2950)],
                       [_cell("الأهداف الظاهرة", width=1600, fill=LIGHT_BLUE, bold=True), _cell("، ".join(record.get("event_targets", [])[:12]) or "-", width=7500, align="right", colspan=3)],
                       [_cell("تفاصيل الحدث", width=1600, fill=LIGHT_BLUE, bold=True), _cell(_arabic_detail(record.get("detail")), width=7500, align="right", colspan=3, size=18)],
                       [_cell("نتائج التحليل المرتبطة بالحدث", width=1600, fill=LIGHT_BLUE, bold=True), _cell(reputation_details, width=7500, align="right", colspan=3)]]
        # Let Word paginate consecutive event cards naturally. Body rows may
        # split when necessary, avoiding a mostly empty page for every event.
        parts.extend([_table(detail_rows, [1600, 2950, 1600, 2950]), _paragraph("", after=35)])
    return "".join(parts)
...
def _recommendations_section(analysis: dict[str, Any]) -> str:
    ...
    if metadata.get("data_type") == "ads_events":
        recommendations.extend(["البدء بالأحداث الأعلى درجةً وحجماً، والتحقق من المستخدم أو الخدمة والوجهة والتوقيت المرتبط بكل حدث.",
                                "مقارنة نشاط النقل مع النسخ الاحتياطي والمهام المجدولة وتبادل الملفات المصرح به قبل اعتباره حادثاً أمنياً.",
                                "إنشاء خط أساس لكل مصدر لرصد الانحرافات في الحجم وعدد الوجهات والمنافذ والفترات الزمنية.",
                                "تجميع الأحداث المتكررة للمصدر نفسه في قضية تحقيق واحدة لتقليل الضوضاء وتحسين فهم التسلسل الزمني."])
    else:
        recommendations.extend(["مراجعة أعلى المصادر والوجهات من حيث حجم البيانات وربطها بالأصول والخدمات المعتمدة.",
                                "التحقق من المنافذ غير المعتادة والتدفقات ذات الحجم الكبير أو عدد الحزم غير المتناسب.",
                                "إنشاء خط أساس زمني لحركة الشبكة ومقارنة النتائج الحالية بالفترات السابقة."])
    recommendations.append("توثيق قرار الإغلاق أو التصعيد لكل نتيجة مع ذكر الأدلة والمبرر التشغيلي واسم المحلل وتاريخ المراجعة.")
    ...
    return "".join([_section_title("التوصيات"), *items, _section_title("الخلاصة"),
                    ...
                    _paragraph("فريق تحليل أمن الشبكة - FlowScope", bold=True, size=21)])
...
def _header_xml() -> str:
    return ('<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'
            ...
            f'{_run("تقرير تحليل أمن الشبكة | FlowScope", bold=True, color=BLUE, size=18)}</w:p></w:hdr>')
